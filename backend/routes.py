...
def songs():
    cursor = db.songs.find({})  # Get the MongoDB cursor
    list_songs = list(cursor)  # Convert the cursor to a list
    json_songs = json.loads(dumps(list_songs))  # Convert MongoDB documents to JSON serializable format
    return jsonify({"songs": json_songs}), 200
